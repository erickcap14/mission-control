...
// ---------------------------------------------------------------------------
// Toolkit (per-device scanner snapshots)
// ---------------------------------------------------------------------------

/**
 * Upserts the toolkit snapshot for one device.
 * @param {string} deviceId
 * @param {object} data - full scanToolkit result ({ skills, mcpServers, plugins, globalSettings })
 */
export async function upsertToolkit(deviceId, data) {
  await getPool().query(
    `INSERT INTO device_toolkit (device_id, data, updated_at)
     VALUES ($1, $2, now())
...
/**
 * Returns all stored toolkit rows.
 * @returns {Promise<Array<{device: string, data: object, updatedAt: Date}>>}
 */
export async function getToolkits() {
  const { rows } = await getPool().query(
    `SELECT dt.device_id, dt.data, dt.updated_at, d.name AS device_name
       FROM device_toolkit dt
