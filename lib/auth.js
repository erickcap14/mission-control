...
const failures = new Map(); // ip -> { count, lockedUntil, firstAt }

function pruneFailures(now) {
  for (const [ip, entry] of failures) {
    if (entry.lockedUntil < now && now - entry.firstAt > LOCKOUT_MS) failures.delete(ip);
  }
}
...
function recordLoginFailure(ip) {
  const now = Date.now();
  const entry = failures.get(ip) || { count: 0, lockedUntil: 0, firstAt: now };
  entry.count++;
...
export function loginHandler(req, res) {
  ...
  const lockedMs = loginLockedFor(ip);
  if (lockedMs > 0) {
