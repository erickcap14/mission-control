...
/**
 * Reads a command file and returns { description, type }.
 * Files with a YAML `description:` frontmatter block are "skill" (registered
 * with the Claude Code Skill tool). Files without it are plain "command"
 * templates invoked via slash syntax only.
 */
async function readCommandMeta(filePath) {
  try {
    ...
    if (lines[0].trim() === '---') {
      ...
      if (closeIdx > 0) {
        ...
        if (/^description:/m.test(frontmatter)) {
          ...
          const desc = descLine ? descLine.replace(/^description:\s*/, '').trim() : '';
          return { description: desc, type: 'skill' };
        }
      }
    }

    // No frontmatter — plain command; use first non-empty line as description
    for (const line of lines) {
      const trimmed = line.trim();
      if (trimmed && trimmed !== '---') {
        return { description: trimmed.replace(/^#+\s*/, ''), type: 'command' };
      }
    }
  } catch {
    // unreadable
  }
  return { description: '', type: 'command' };
}
...
async function scanSkills(scanPath, usageCounts) {
  ...
  for (const [name, { filePath, sources }] of skillMap) {
    const { description, type } = await readCommandMeta(filePath);
    results.push({
      name,
      type,
      description,
...
/**
 * Extracts mcpServers entries from a settings.json or .mcp.json file.
 */
async function extractMCPFromFile(filePath, source) {
  const entries = [];
  try {
    ...
    for (const [name, cfg] of Object.entries(servers)) {
      ...
      entries.push({ name, command: cmd.slice(0, 80), source });
    }
  } catch {
...
/**
 * Scans MCP server definitions from .mcp.json files and settings.json mcpServers keys.
 */
async function scanMCPServers(scanPath) {
  const serverMap = new Map(); // name -> { command, sources[] }

  function merge(name, command, source) {
    if (serverMap.has(name)) {
      const entry = serverMap.get(name);
      if (!entry.sources.includes(source)) entry.sources.push(source);
    } else {
      serverMap.set(name, { command, sources: [source] });
    }
  }

  // Global settings
  const globalSettings = path.join(os.homedir(), '.claude', 'settings.json');
  for (const { name, command, source } of await extractMCPFromFile(globalSettings, 'global')) {
    merge(name, command, source);
  }

  // Per-project .mcp.json and settings.json
  let projects = [];
  try {
    projects = await discoverProjects(scanPath);
  } catch {
  ...
  for (const project of projects) {
    const mcpJson = path.join(project.path, '.mcp.json');
    for (const { name, command, source } of await extractMCPFromFile(mcpJson, project.name)) {
      merge(name, command, source);
    }
    const settingsJson = path.join(project.path, '.claude', 'settings.json');
    for (const { name, command, source } of await extractMCPFromFile(settingsJson, project.name)) {
      merge(name, command, source);
    }
  }

  return Array.from(serverMap.entries()).map(([name, { command, sources }]) => ({
    name,
    type: 'mcp',
    description: command,
    sources,
...
export async function scanToolkit(config) {
  ...
  const [skills, hooks, mcpServers, plugins, globalSettings] = await Promise.all([
    scanSkills(resolvedScanPath, usageCounts),
    scanHooks(resolvedScanPath),
    scanMCPServers(resolvedScanPath),
    scanPlugins(),
    scanGlobalSettings(),
  ]);

  return { skills: [...skills, ...hooks], mcpServers, plugins, globalSettings };
}
